...
**Purpose:** Short, visual task guides that help users reach a specific goal.

The samples document MuseScore 3.6.2, a real open-source music notation program, and all screenshots were taken in that version. MuseScore 4 has different menus and toolbars, so some steps do not apply to it.
...
- Numbered, task-oriented steps, each with the result the user should see
- Screenshots that show the control or the outcome of a step
- Notes and warnings placed where mistakes are likely
- Plain language for users without a music-software background
- Covers the practical tasks for a first score, not every feature
